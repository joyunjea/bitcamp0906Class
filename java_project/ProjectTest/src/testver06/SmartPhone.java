...
public class SmartPhone {
	...
	public void showMenu() {
		while (true) {
			...
			while (true) {
				try {
					select = misInfo.selectMenu(Integer.parseInt(sc.nextLine()));
					break;
	...
	public void insert() {
		...
		if (choice == 1) {
			...
			String name = getName(sc.nextLine());
...
//      }
	...
	public void update() {
		...
		if (index < 0) {
			...
			if (c instanceof CompanyContact) {
				...
				// 새 정보를 입력받습니다.
				System.out.println("이름>>");
				String newName = getName(sc.nextLine());
	...
	private String getPhone() {
		...
		while (true) {
			...
			} else if (checkPhoneForm(phoneNumber)) {
	...
	// 형식에 맞지 않으면 true 맞으면 false 를 반환하는 메소드
	private boolean checkPhoneForm(String number) {

		return !number.matches("^(010)-?[0-9]{3,4}-?[0-9]{4}$");
	...
	private String getName(String name) {
		...
		while (true) {
			if (checkName(getInfo(result))) {
				break;

			} else {
				
				System.out.println("이름은 한글, 영문 대소문자만 허용합니다.");
				result = sc.nextLine();
			}
		}
	...
	// 한글, 영어로만 이루어지지 않았을때 false 반환
	private boolean checkName(String name) {
		return name.matches("^[A-Z a-z 가-힣]*$");
	}
