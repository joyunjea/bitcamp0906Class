...
//	 * ② 정수 두 개를 매개변수의 인자로 전달받아 빼기연산 후 출력하는 메소드를 정의
	public void minus(int num1, int num2) {
		long result = (long)num1 - num2;
		System.out.println(num1+"-"+num2+"="+(num1-num2)); //문자열-문자열은 에러이므로 괄호표현
	}
//	 * ③ 정수 두 개를 매개변수의 인자로 전달받아 곱하기연산 후 출력하는 메소드를 정의 
	public void multyply(int num1, int num2) {
		long result = (long)num1 * num2;
		System.out.println(num1+"*"+num2+"="+result);
	}
...
//	 * ⑥ 실수 반지름 하나를 매개변수의 인자로 전달받아 원의 넓이를
//	 * 구해 반환하는 메소드를 반환하는 메소드를 정의 원의 둘레 : 2 x π x r , 월의 넓이 : π x r x r 
	float cirArea(float r) {
		return PI * r * r;
	}
//	 * ⑦ main() 메소드를 정의하고 각각의 메소드를 호출해서 결과를 콘솔에 출력해봅시다. 
	public static void main(String[] args) {
		Calculator cal = new Calculator("손흥민");	//null로 초기화되지만 생성자로 값을 넣어줄수있다
...
//		 * 데이터를 메소드의 매개변수의 인자로 전달하는 코드를 main() 메소드 에 추가해봅시다.
		Scanner scanner = new Scanner(System.in);     //생성자의 오버로딩
