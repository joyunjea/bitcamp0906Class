...
-- null 값을 치환해주는 nvl함수
-- nvl(컬럼명, 치환할 데이터) = > 컬럼의 값중 null 값은 치환 값으로
select comm, nvl(comm, 0)
from emp;

-- nvl 함수를 이용해서 결과 값을 도출
...
select ename as name, sal as sal, sal * 12 + nvl(comm, 0)as salofyear
from emp;
...
--사원들이 소속된 부서 번호 출력 distinct 중복되는 부분을 한번만 출력해줌
...
--이름이 N으로 끝나는 데이터를 찾아봅시다. 
select *
from emp
where ename like '%N';

-- 이름의 두 번째 글자가 A인 사원을 찾는
select *
from emp
where ename like '_A%';
...
-- 급여 컬럼을 기준으로 오름차순으로 정렬한 예입니다. 
select *
from emp
--order by sal asc;
--order by sal;    -- 기본 정렬 : asc(오름차순)이다
order by sal desc;
